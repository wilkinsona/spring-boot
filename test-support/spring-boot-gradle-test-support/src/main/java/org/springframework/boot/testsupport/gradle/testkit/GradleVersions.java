...
public final class GradleVersions {
	...
	public static List<String> allCompatible() {
		if (isJavaVersion(JavaVersion.VERSION_24)) {
			return Arrays.asList("8.14.3", GradleVersion.current().getVersion());
		}
		if (isJavaVersion(JavaVersion.VERSION_23)) {
			return Arrays.asList("8.14.3", GradleVersion.current().getVersion());
		}
		if (isJavaVersion(JavaVersion.VERSION_22)) {
			return Arrays.asList("8.8", "8.14.3", GradleVersion.current().getVersion());
		}
		if (isJavaVersion(JavaVersion.VERSION_21)) {
			return Arrays.asList("8.5", "8.14.3", GradleVersion.current().getVersion());
		}
		return Arrays.asList("7.6.6", "8.4", "8.14.3", GradleVersion.current().getVersion());
	}
