...
class GrpcDisableCsrfHttpConfigurer extends AbstractHttpConfigurer<GrpcDisableCsrfHttpConfigurer, HttpSecurity> {
	...
	public void init(HttpSecurity http) {
		...
		if (context != null && isCsrfConfigurerPresent(http) && hasBean(context, GrpcServiceDiscoverer.class)
				&& hasBean(context, GrpcServletRegistration.class) && isCsrfDisabled(context)) {
	...
	private boolean isCsrfDisabled(ApplicationContext context) {
	...
	private void disable(CsrfConfigurer<HttpSecurity> csrf) {
		RequestMatcher notToAnyGrpcService = new NegatedRequestMatcher(GrpcRequest.toAnyService());
		csrf.requireCsrfProtectionMatcher(new AndRequestMatcher(CsrfFilter.DEFAULT_CSRF_MATCHER, notToAnyGrpcService));
	}
