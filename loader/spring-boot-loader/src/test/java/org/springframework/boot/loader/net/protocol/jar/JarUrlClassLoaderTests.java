...
class JarUrlClassLoaderTests {
	...
	void enableLoaderCache() throws Exception {
		...
		resolveFromNested();
		try (JarUrlClassLoader loader = new TestJarUrlClassLoader(
				new URL("jar:file:src/test/resources/jars/app.jar!/"))) {
			loader.setEnableCache(true);
			assertThat(loader.getResource("demo/Application.java"))
				.isEqualTo(loader.getResource("demo/Application.java"));
