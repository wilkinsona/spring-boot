...
By default, xref:data/nosql.adoc#data.nosql.mongodb.observability[auto-configuration of the MongoDB driver's observation support] is preferred.
To use command metrics instead, set configprop:management.metrics.mongodb.command.enabled[] to `true`.
This will trigger registration of a javadoc:io.micrometer.core.instrument.binder.mongodb.MongoMetricsCommandListener[] with the auto-configured javadoc:{url-mongodb-driver-sync-javadoc}/com.mongodb.client.MongoClient[].
