...
class CodecsAutoConfigurationTests {
	...
	@Test
	void loggingRequestDetailsCustomizerShouldUseHttpCodecProperties() {
		this.contextRunner.withPropertyValues("spring.http.codec.log-request-details=true")
			.run((context) -> assertThat(defaultCodecs(context))
				.hasFieldOrPropertyWithValue("enableLoggingRequestDetails", true));
	}
	...
	void logRequestDetailsShouldGivePriorityToHttpCodecProperty() {
		this.contextRunner
			.withPropertyValues("spring.http.codec.log-request-details=true", "spring.codec.log-request-details=false")
			.run((context) -> assertThat(defaultCodecs(context))
				.hasFieldOrPropertyWithValue("enableLoggingRequestDetails", true));
	}
	...
	void maxInMemorySizeShouldUseHttpCodecProperties() {
		this.contextRunner.withPropertyValues("spring.http.codec.max-in-memory-size=64KB")
			.run((context) -> assertThat(defaultCodecs(context)).hasFieldOrPropertyWithValue("maxInMemorySize",
					64 * 1024));
	}
	...
	void maxInMemorySizeShouldGivePriorityToHttpCodecProperty() {
		this.contextRunner
			.withPropertyValues("spring.http.codec.max-in-memory-size=64KB", "spring.codec.max-in-memory-size=32KB")
			.run((context) -> assertThat(defaultCodecs(context)).hasFieldOrPropertyWithValue("maxInMemorySize",
					64 * 1024));
	}
