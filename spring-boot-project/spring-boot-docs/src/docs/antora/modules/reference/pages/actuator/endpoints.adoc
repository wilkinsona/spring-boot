...
If you prefer access to be opt-in rather than opt-out, set the configprop:management.endpoints.default-access[] property to `disabled` and use individual endpoint `access` properties to opt back in.
...
Application-wide endpoint access can be limited using the configprop:management.endpoints.max-permitted-access[] property.
