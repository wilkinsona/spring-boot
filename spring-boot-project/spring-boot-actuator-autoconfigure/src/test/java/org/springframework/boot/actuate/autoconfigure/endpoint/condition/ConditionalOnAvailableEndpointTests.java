...
class ConditionalOnAvailableEndpointTests {
	...
	void whenBothAccessAndEnabledAreConfiguredThenThrows() {
		this.contextRunner
			.withPropertyValues("management.endpoints.web.exposure.include=*",
					"management.endpoint.shutdown.enabled=true", "management.endpoint.shutdown.access=disabled")
	...
	void whenBothDefaultAccessAndDefaultEnabledAreConfiguredThenThrows() {
		this.contextRunner
			.withPropertyValues("management.endpoints.web.exposure.include=*",
					"management.endpoints.enabled-by-default=true", "management.endpoints.default-access=disabled")
			.run((context) -> assertThat(context).hasFailed()
				.getFailure()
				.rootCause()
				.isInstanceOf(MutuallyExclusiveConfigurationPropertiesException.class));
	...
	@Endpoint(id = "shutdown", defaultAccess = Access.DISABLED)
