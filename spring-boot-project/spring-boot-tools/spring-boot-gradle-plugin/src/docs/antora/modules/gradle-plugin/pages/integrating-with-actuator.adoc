...
This will configure a {apiref-gradle-plugin-boot-build-info}[`BuildInfo`] task named `bootBuildInfo` and, if it exists, make the Java plugin's `processResources` task include the output of the `bootBuildInfo` task in `META-INF`.
The task's destination directory will be `build/generated/bootBuildInfo`.
